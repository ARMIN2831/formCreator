<div @if($edit) wire:click="showEdit({{ $field->id }})" @click="edit = false , menu = true" @endif class="relative group px-1 pb-1 cursor-pointer @if($edit) hover:bg-slate-100 transition-all ease-in-out duration-100 @endif">
    <label class="flex flex-row text-sm font-semibold text-slate-500 items-center cursor-pointer">
        {{ $field->value }}:
        <input value="{{ $field->placeholder }}" wire:model="arr.{{ $field->id }}" name="{{ $field->placeholder }}" type="radio" class="shadow h-8 rounded-md mx-2" placeholder="{{ $field->placeholder }}">
